...
public partial class FieldStaff : System.Web.UI.Page
{
   ...
   private readonly string senderEmail = "enter email";
        ...
        try
        {
            using (SqlConnection con = new SqlConnection(connString))
            {
                ...
                using (SqlCommand cmd = new SqlCommand(query, con))
                {
                    ...
                    if (rowsAffected > 0)
                    {
                        // 🔥 Fire Welcome Email to New Staff
    ...
    // ==========================================
    // 🔥 NEW EMAIL LOGIC (Staff Welcome Mail)
    // ==========================================
        ...
        try
        {
            using (SqlConnection con = new SqlConnection(connString))
            {
                string query = "SELECT StaffID, FullName, ContactNo, Email, IsActive " +
                               "FROM tbl_staff WHERE Department = 'Water' ORDER BY StaffID DESC";
    ...
    protected void rptStaff_ItemCommand(object source, RepeaterCommandEventArgs e)
    {
        ...
        if (e.CommandName == "ToggleStatus")
        {
            ...
            if (currentStatus)
            {
                litModalIcon.Text = "<div class='w-full h-full bg-blue-100 text-blue-500 rounded-full flex items-center justify-center'><i class='fa-solid fa-user-lock'></i></div>";
                ...
                btnModalConfirm.CssClass = "flex-1 bg-blue-600 hover:bg-blue-700 text-white font-bold py-2.5 rounded-lg shadow-md transition cursor-pointer text-sm";
