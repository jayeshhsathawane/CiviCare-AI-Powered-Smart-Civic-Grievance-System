...
public partial class ElectricFieldStaff : System.Web.UI.Page
{
    ...
    protected void btnAddStaff_Click(object sender, EventArgs e)
    {
        ...
        string email = txtEmail.Text.Trim(); // Fetching the new Email field
    ...
    // ==========================================
    // 2. BIND STAFF TABLE (Including Email)
    // ==========================================
    private void BindStaffGrid()
    {
        try
        {
            using (SqlConnection con = new SqlConnection(connString))
            {
                // Selecting Email to display in Grid
                string query = "SELECT StaffID, FullName, ContactNo, Email, IsActive " +
                               "FROM tbl_staff WHERE Department = 'Electric' ORDER BY StaffID DESC";
