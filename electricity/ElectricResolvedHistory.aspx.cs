        ...
        try
        {
            using (SqlConnection con = new SqlConnection(connString))
            {
                string query = "SELECT c.ComplaintID, c.UserDepartment, c.Description, c.LocationName, " +
                               ...
                               "WHERE c.AssignedDepartment = 'Electric' AND c.Status = 'Resolved' ";
                ...
                using (SqlCommand cmd = new SqlCommand(query, con))
                {
                    ...
                    if (endDate.HasValue)
                        cmd.Parameters.AddWithValue("@EndDate", endDate.Value.Date.AddDays(1).AddSeconds(-1)); // Include entire end day
